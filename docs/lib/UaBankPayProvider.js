...
import { DEFAULT_CATEGORY, LOCK_ALL_FIELDS_MASK, MAX_DESTINATION_LENGTH, MAX_DISPLAY_LENGTH, MAX_RECEIVER_CODE_LENGTH, MAX_RECEIVER_NAME_LENGTH, MAX_REFERENCE_LENGTH, NBU_QR_BASE_URL, NBU_QR_SPEC_VERSION } from './constants.js';
...
function validateAmount(amount) {
    const value = (amount !== null && amount !== void 0 ? amount : '').trim();
    if (value === '') {
...
function buildPaymentData(request) {
    var _a;
    ...
    const changeable = (_a = request.changeable) !== null && _a !== void 0 ? _a : true;
    return [
        'BCD', // Service tag
 ...
 */
export function generatePayLink(request) {
    const paymentData = buildPaymentData(request);
    // Encode the payment data string into a Uint8Array and then convert it to a Base64 URL-safe string
    const encoder = new TextEncoder();
    const structureBytes = encoder.encode(paymentData);
    const base64Url = encodeBase64Url(structureBytes);
...
export class UaBankPayProvider {
     ...
     * @deprecated Use the standalone {@link generatePayLink} function instead.
     */
    static generatePayLink(request) {
        return generatePayLink(request);
    }
}
//# sourceMappingURL=UaBankPayProvider.js.map